...
file_name = "../homecredit.json"  # Ensure this file is in the same directory as your script
...
output_file = os.path.join(output_dir, "homecredit.txt")
with open("generatedText/homecredit.txt", "w") as file:
    file.write(generated_purpose)

print("Output saved to homecredit.txt")

# Reference metadata for BLEU score calculation
reference = [
    [
        "to", "develop", "machine", "learning", "models", "that", "predict", 
        "client", "default", "risk", "helping", "finance", "providers", 
        "more", "reliably", "assess", "loan", "applicants", "with", "little", 
        "or", "no", "credit", "history", "while", "ensuring", "model", "stability"
    ]
]
